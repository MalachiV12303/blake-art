...
const direction = ref<"next" | "prev">("next"); // track slide direction
...
const handleTouchEnd = (e: TouchEvent) => {
    ...
    if (Math.abs(diffX) > 50) {
        if (diffX < 0) showNext(); // swipe left → next
        else showPrev(); // swipe right → prev
    }
};
const getFormattedIndex = (number: number) => {
...
<template>
    ...
    <div v-if="selectedIndex !== null" class="fullscreen-image" @click="closeImage" @touchstart="handleTouchStart"
        ...
        <button class="nav-btn left zalando-sans-expanded" @click.stop="showPrev">
            ‹
        </button>

        <div class="counter">
            <span>
                {{ getFormattedIndex(selectedIndex + 1) }}/{{ items.length }}
            </span>
        </div>
        ...
        <button class="nav-btn right zalando-sans-expanded" @click.stop="showNext">
            ›
        </button>
    </div>
</template>
